...
(function(module, process, console) {
  ...
  var hl7Parser = function(options) {
    ...
    if (typeof this.options.fs === 'undefined') {
      ...
      this.options.fs = require(this.options
        .fs); /* @deprecated: dangerous, I will consider not support this functionality */
    }
  };
  ...
  function getDelimiters(mshsegment) {
    ...
    return {
      composite: mshsegment.substring(3, 4), // | = field
      subComposite: mshsegment.substring(4, 5), // ^ = component
      repetitions: mshsegment.substring(5, 6), // ~ = repeat
      escapeChar: mshsegment.substring(6, 7), // \ = escape
      subComponent: mshsegment.substring(7, 8) // & = subcomponent
    };
  }

  /**
   * @param {string} text - this text to be processed
   * @param {{key: value}} equivalences - key is the string to replace, value is the replacement
   * @returns {string} - return an escaped string
   */
  ...
  /**
   * Check the name of segment is parsed segment names
  ...
  /**
   * Test if a new line in hl7 message continues the previous line as a previous
   * segment or starts a new segment. 
   * @param {string} typeofSegment 
   * @param {string[]} parts 
   * @param {boolean} isFirst 
   */
  function isRecoverable(typeofSegment, parts, isFirst) {
    return (
      ((parts.length > 0 && endsWith(parts[parts.length - 1], '\\X000d\\')) ||
        endsWith(typeofSegment, '\\X000d\\')) &&
      !isFirst
    );
  }

  /**
   * replace predefined to escape characters in the string 
   * @param {string} string 
   */
  ...
  hl7Parser.prototype.parse = function(messageContent, ID, wrappedDone) {
    ...
    if (segmentslines.length === 0) {
      ...
      var equivalences = [
        {
          key: new RegExp(
            escapeRegExp(delimiters.escapeChar + 'F' + delimiters.escapeChar),
            'g'
          ),
        ...
        {
          key: new RegExp(
            escapeRegExp(delimiters.escapeChar + 'S' + delimiters.escapeChar),
            'g'
          ),
        ...
        {
          key: new RegExp(
            escapeRegExp(delimiters.escapeChar + 'R' + delimiters.escapeChar),
            'g'
          ),
        ...
        {
          key: new RegExp(
            escapeRegExp(delimiters.escapeChar + 'E' + delimiters.escapeChar),
            'g'
          ),
        ...
        {
          key: new RegExp(
            escapeRegExp(delimiters.escapeChar + 'T' + delimiters.escapeChar),
            'g'
          ),
        ...
        },
        {
          key: new RegExp(
            escapeRegExp(
              delimiters.escapeChar + 'X000d' + delimiters.escapeChar
            ),
            'g'
          ),
          value: '\r'
        ...
        {
          key: new RegExp(
            escapeRegExp(delimiters.escapeChar + 'X0d' + delimiters.escapeChar),
            'g'
          ),
          value: '\r'
        }
      ...
      var previousTypeOfSegment = '',
        previousparts = [],
        previousorder = -1;
      for (
        var segmentnumber = 0, segmentmax = segmentslines.length;
        segmentnumber < segmentmax;
        segmentnumber++
      ) {
        ...
        if (line !== '') {
          ...
          if (parts.length > 0) {
            ...
            if (!validSegmentType(typeofSegment, ID, self.logger)) {
              if (
                ...
                result[result.length - 1] = {
                  segmentType: previousTypeOfSegment,
                  order: previousorder,
                  parts: previousparts
                };
                continue;
            ...
            for (
              var numberOfPart = 0, numberOfParts = parts.length;
              ...
              if (
                ((segmentnumber === 0 && numberOfPart !== 0) ||
                  segmentnumber !== 0) &&
                part.indexOf(delimiters.subComposite) >= 0
              ) {
                var subdivisions = part.split(delimiters.subComposite);
                for (
                  var subdivisionsIdx = 0,
                    numberOfSubdivisions = subdivisions.length;
                  subdivisionsIdx < numberOfSubdivisions;
                  subdivisionsIdx++
                ) {
                  subdivisions[subdivisionsIdx] = escapeChars(
                    subdivisions[subdivisionsIdx],
                    equivalences
                  );
                }
            ...
            result.push(
              new HL7Segment(typeofSegment, previousorder + 1, parts)
            );
      ...
      r.segments.forEach(s => s.parseFields(delimiters));
  ...
  Hl7Message.prototype.getValue = function(path) {
    let segmentName = path.split('.').shift();
    const idx = this.segments.findIndex(v => v.typeofSegment === segmentName);
    if (!~idx) {
      return null;
    }
    return this.segments[idx].getValue(path);
  }
  ...
  Hl7Message.prototype.getSegmentAt = function(counter) {
    return typeof this.segments[counter] === 'object'
      ? this.segments[counter]
      : null;
  };
  Hl7Message.prototype.size = function() {
    return this.segments.length;
  };
  Hl7Message.prototype.getSegments = function(
    segmentName,
    nmbr,
    fieldName,
    joinChar
          ...
          function(fail, readBytes) {
            ...
            if (readBytes > 0) {
              var msg =
                fileEncoding !== 'utf8'
                  ? encoding
                      .convert(readBuffer, 'utf8', fileEncoding)
                      .toString('utf8')
  ...
  /**
   * Parse fields which might have repeat/component/subcomponent
   */
  HL7Segment.prototype.parseFields = function(delimiters) {
    if (!this.parts) {
      return;
    }
    // reverse the parsing of parts made by previous parse
    let fields = this.parts.map(p => Array.isArray(p) ? p.join(delimiters.subComposite) : p)
    // parse repeat first then component(subComposite) then subcomponent.
    fields = fields.map(f => f.split(delimiters.repetitions).map(r => {
      if (r === '') {
        return null;
      }
      return r.split(delimiters.subComposite).map(s => {
        if (s === '') {
          return null;
        }
        return s.split(delimiters.subComponent)
      })
    }));
    this.fields = fields;
  }
  ...
  /**
   * parse idx of path element 
   * F-1 -> returns 1
   * R-1 -> returns 1
   * @param {string} path
   * @returns {number}
   */
  function parsePathIdx(path){
    if (!path) {
      return new Error('invalid path element: ' + path);
    }
    let t = path.split('-');
    if (t.length !== 2) {
      return new Error('invalid path element: ' + path);
    }
    let rslt = parseInt(t[1]);
   ...
   */
  HL7Segment.prototype.getValue = function(path) {
    if (!path) {
      return null;
    }
    if (!this.fields) {
      throw new Error('segment not parsed!')
    }
    let paths = path.split('.');
    paths.shift(); // remove element name
    let itr = this.fields;
    for (let i = 0 ; i < paths.length; i++){
      let idx = parsePathIdx(paths[i]) - 1;
      if (itr.length <= idx || idx < 0) {
        return null;
      } 
      itr = itr[idx]
      if (itr === null) {
    ...
    return itr;
    // path is shorter than parsed value, return by default the first value
    // path is longer than parsed value, return null by default
  }
  ...
  HL7Segment.prototype.get = function(nameField, joinChar) {
    ...
    if (typeof this.segmentsFields[this.typeofSegment] !== 'undefined') {
      ...
      if (
        idx >= 0 &&
        typeof joinChar !== 'undefined' &&
        typeof this.parts[idx] === 'object'
      ) {
  ...
  function getSegmentsInformation() {
    ...
    var segmentsinfo = files
      .filter(function(filename) {
        return filename.endsWith('.json');
      })
      .map(function(filename) {
        return String(
          fs.readFileSync(path.join(__dirname, 'segments', filename), 'utf8')
        );
      })
